...
def run_test(server_address, operation, file_size_mb, num_workers, use_process_pool=False, csv_filename="results.csv"):
    ...
    # Cek kalau file belum ada atau kosong, tulis header dulu
    write_header = not os.path.exists(csv_filename) or os.stat(csv_filename).st_size == 0

    with open(csv_filename, "a", newline="") as csvfile:
        fieldnames = ['operation', 'file_size_mb', 'client_workers', 'worker_id', 'success', 'time', 'bytes']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        if write_header:
            writer.writeheader()

        with executor_class(max_workers=num_workers) as executor:
            futures = [executor.submit(worker_func, client, file_size_mb, i) for i in range(num_workers)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv-file', default='results.csv', help='CSV filename to save results')
